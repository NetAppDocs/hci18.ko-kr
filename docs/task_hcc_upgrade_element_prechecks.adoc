...
명령을 사용하여 스토리지 클러스터를 업그레이드할 준비가 되었는지 확인할 수 `sfupgradecheck` 있습니다. 이 명령은 보류 중인 노드, 디스크 공간 및 클러스터 장애 등의 정보를 확인합니다.

관리 노드가 어두운 사이트에 있는 경우 업그레이드 준비 검사를 `metadata.json` 실행하려면 동안 다운로드한 파일이 link:task_upgrade_element_latest_healthtools.html["HealthTools 업그레이드"]필요합니다.

.이 작업에 대해
이 절차에서는 다음 결과 중 하나를 생성하는 업그레이드 검사를 처리하는 방법에 대해 설명합니다.

*  `sfupgradecheck`명령을 실행하면 성공적으로 실행됩니다. 클러스터를 업그레이드할 준비가 되었습니다.
* 도구 내 `sfupgradecheck` 검사가 실패하고 오류 메시지가 표시됩니다. 클러스터를 업그레이드할 준비가 되지 않았습니다. 추가 단계가 필요합니다.
* HealthTools가 최신 상태가 되었다는 오류 메시지와 함께 업그레이드 확인이 실패합니다.
* 관리 노드가 어두운 사이트에 있기 때문에 업그레이드 확인에 실패했습니다.


.단계
.  `sfupgradecheck`다음 명령을 실행합니다.
+
[listing]
----
...
NOTE: 특수 문자가 포함된 암호의 경우 (`\`각 특수 문자 앞에 백슬래시)를 추가합니다. 예를 들어, 는 `mypass!@1` 로 `mypass\!\@`입력해야 합니다.
...
----
이 예에서 노드 1의 디스크 공간이 부족합니다. 자세한 내용은 https://kb.netapp.com["기술 자료"^] 오류 메시지에 나열된 (KB) 문서를 참조하십시오.
...
----
. 자세한 내용은 다크 사이트에 대한 추가 link:task_upgrade_element_latest_healthtools.html["HealthTools 업그레이드"] 정보를 참조하십시오.
