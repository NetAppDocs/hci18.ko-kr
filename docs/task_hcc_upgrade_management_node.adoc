...
[role="lead"]
관리 노드를 버전 11.0 이상에서 관리 노드 버전 12.2로 업그레이드할 수 있습니다.
...
.필요한 것
* vCenter Plug-in 4.4 이상에서는 모듈식 아키텍처를 사용하여 생성되고 개별 서비스를 제공하는 관리 노드 11.3 이상이 필요합니다.


.업그레이드 옵션
다음 관리 노드 업그레이드 옵션 중 하나를 선택합니다.

* 관리 노드 12.0에서 업그레이드하는 경우: <<관리 노드를 12.0에서 버전 12.2로 업그레이드합니다>>
* 관리 노드 11.3, 11.5, 11.7 또는 11.8에서 업그레이드하는 경우: <<관리 노드를 11.3에서 11.8까지 버전 12.2로 업그레이드합니다>>
* 관리 노드 11.0 또는 11.1에서 업그레이드하는 경우: <<관리 노드를 11.1 또는 11.0에서 버전 12.2로 업그레이드합니다>>
...
== 관리 노드를 12.0에서 버전 12.2로 업그레이드합니다

새 관리 노드 가상 컴퓨터를 프로비저닝할 필요 없이 버전 12.0에서 버전 12.2로 관리 노드 전체 업그레이드를 수행할 수 있습니다.


NOTE: Element 12.2 관리 노드는 선택적 업그레이드입니다. 기존 배포에는 필요하지 않습니다.

.필요한 것
* 업그레이드하려는 관리 노드는 버전 12.0이며 IPv4 네트워킹을 사용합니다. 관리 노드 버전 12.2는 IPv6을 지원하지 않습니다.
+
...
* 관리 노드를 버전 12.2로 업데이트하는 경우 계속하려면 관리 서비스 2.14.60 이상이 필요합니다.
* 의 지침을 사용하여 추가 네트워크 어댑터(필요한 경우)를 구성했습니다.link:task_mnode_install_add_storage_NIC.html["추가 스토리지 NIC 구성"]
+
...
. NetApp 지원 사이트에서 관리 노드 가상 머신으로 NetApp HCI용 를 https://mysupport.netapp.com/site/products/all/details/netapp-hci/downloads-tab["관리 노드 ISO"^] 다운로드합니다.
+
...
----
. (영구 볼륨이 없는 구성에만 해당) 백업할 컨테이너 폴더의 내용을 복사합니다.
+
[listing]
----
...
. (영구 볼륨이 없는 구성에만 해당) 컨테이너 폴더의 내용을 다시 원래 위치로 이동합니다.
+
[listing]
----
sudo su
...
IMPORTANT: 이전에 관리 노드에서 SSH 기능을 비활성화한 경우에는 복구된 관리 노드에서 작업해야 합니다link:task_mnode_ssh_management.html["SSH를 다시 비활성화합니다"]. 에서 제공하는 SSH link:task_mnode_enable_remote_support_connections.html["NetApp RST(Remote Support Tunnel) 세션 액세스"]기능은 기본적으로 관리 노드에서 사용하도록 설정됩니다.



== 관리 노드를 11.3에서 11.8까지 버전 12.2로 업그레이드합니다

새 관리 노드 가상 머신을 프로비저닝할 필요 없이 버전 11.3, 11.5, 11.7 또는 11.8에서 버전 12.2로 관리 노드를 데이터 이동 없이 업그레이드할 수 있습니다.


NOTE: Element 12.2 관리 노드는 선택적 업그레이드입니다. 기존 배포에는 필요하지 않습니다.

.필요한 것
* 업그레이드하려는 관리 노드는 버전 11.3, 11.5, 11.7 또는 11.8이며 IPv4 네트워킹을 사용합니다. 관리 노드 버전 12.2는 IPv6을 지원하지 않습니다.
+
...
== 관리 노드를 11.1 또는 11.0에서 버전 12.2로 업그레이드합니다

새 관리 노드 가상 머신을 프로비저닝할 필요 없이 관리 노드를 11.0 또는 11.1에서 버전 12.2로 전체 업그레이드를 수행할 수 있습니다.
...
* 업그레이드하려는 관리 노드는 버전 11.0 또는 11.1이며 IPv4 네트워킹을 사용합니다. 관리 노드 버전 12.2는 IPv6을 지원하지 않습니다.
+

TIP: 관리 노드의 버전을 확인하려면 관리 노드에 로그인하고 로그인 배너에서 Element 버전 번호를 확인합니다. 관리 노드 11.0의 경우 VM 메모리를 수동으로 12GB로 늘려야 합니다.
...
. 12.2 관리 노드에서 `upgrade-mnode` 스크립트를 실행하여 이전 구성 설정을 유지합니다.
+
...
----
.. 영구 볼륨이 없는 기존 관리 노드 11.0 또는 11.1에서 관리되는 여러 스토리지 클러스터의 경우( `-pvm`플래그는 클러스터의 MVIP 주소 중 하나를 제공하는 것임):
+
[listing]
----
...
. (vCenter Server용 NetApp Element 플러그인을 사용하는 모든 NetApp HCI 설치의 경우) 항목의 단계에 따라 12.2 관리 노드에서 vCenter 플러그인을 link:task_vcp_upgrade_plugin.html["vCenter Server용 Element 플러그인을 업그레이드합니다"]업데이트합니다.
. 관리 노드 API를 사용하여 설치의 자산 ID를 찾습니다.
+
.. 브라우저에서 관리 노드 REST API UI에 로그인합니다.
+
... 스토리지 MVIP로 이동하여 로그인합니다. 이렇게 하면 다음 단계에서 인증서를 수락합니다.
...
.. 노드 BIOS 제조업체 및 모델 번호가 나열됩니다. 나중에 사용할 수 있도록 값을 복사하여 `tag` 저장합니다.
...
.. 체험하기 * 를 선택합니다.
.. 클립보드에 복사한 상위 기본 자산 ID를 * asset_id * 필드에 입력합니다.
.. 페이로드에서 Model(모델) 탭에 정의된 대로 필요한 페이로드 값을 입력합니다. 로 `type` 를 입력하고 `ESXi Host` 이전 단계에서 저장한 하드웨어 태그를 붙여 넣습니다 `hardware_tag`.
.. Execute * 를 선택합니다.
...
버전 10.x의 관리 노드가 있는 경우 10.x에서 11.x로 업그레이드할 수 없습니다 대신 이 마이그레이션 절차를 사용하여 구성을 10.x에서 새로 배포된 11.1 관리 노드로 복사할 수 있습니다. 관리 노드가 현재 11.0 이상인 경우 이 절차를 건너뛰어야 합니다. Element 소프트웨어를 10.3+에서 11.x로 업그레이드하려면 관리 노드 11.0 또는 11.1과 최신 HealthTools가 필요합니다
...
NOTE: 기본 암호는 `solidfire` 입니다. 새 암호를 설정하려면 이 암호가 필요합니다.
...
. 관리 노드 IP 주소, 관리 노드 사용자 ID(사용자 이름 `admin`), 등록 유틸리티의 * QoSSIOC Service Management * 탭에 설정한 암호, vCenter 사용자 ID 및 암호를 제공합니다.
+
vSphere에서 * mNode Settings * (mNode 설정 *) 탭은 mNode 상태를 * up * 으로 표시해야 합니다. 이는 관리 노드 11.1이 vCenter에 등록되었음을 나타냅니다.

. 관리 노드 등록 유틸리티에서 (`https://<mNode 11.1 IP address>:9443`* QoSSIOC 서비스 관리 * 에서 SIOC 서비스를 다시 시작합니다.
. 1분 정도 기다린 후 * NetApp Element 구성 * > * mNode 설정 * 탭을 확인합니다. 그러면 mNode 상태가 * Up * 으로 표시됩니다.
+
상태가 * DOWN * 이면 의 권한을 `/sf/packages/sioc/app.properties` 확인합니다. 파일에 파일 소유자의 읽기, 쓰기 및 실행 권한이 있어야 합니다. 올바른 사용 권한은 다음과 같이 표시되어야 합니다.
...
-rwx------
----
. SIOC 프로세스가 시작되고 vCenter에 mNode 상태가 * UP * 로 표시되면 관리 노드에서 서비스에 대한 로그를 확인합니다 `sf-hci-nma`. 오류 메시지가 없어야 합니다.
...
. vCenter에 ONTAP Select가 구성되어 있는 경우 이전 관리 노드에서 관리 노드 버전 11.1 파일로 파일을 `/sf/packages/nma/conf/.ots.properties` 복사하여 NMA에서 ONTAP Select 알림을 구성하고 `.ots.properties` 다음 명령을 사용하여 NMA 서비스를 다시 시작합니다.
+
[listing]
----
...
+
.. SSH를 사용하여 관리 노드 버전 11.1로 이동하고 디렉토리로 이동합니다 `/sf/packages/collector`.
.. 다음 명령을 실행합니다.
+
[listing]
----
...
----
..  `sfcollector`로그를 확인하여 작동하는지 확인합니다.
