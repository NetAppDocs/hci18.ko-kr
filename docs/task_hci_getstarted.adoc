...
* 자세한 내용 및 기타 지원되는 구성은  https://www.netapp.com/pdf.html?item=/media/9413-tr4820.pdf["_TR-4820: NetApp HCI 네트워킹 빠른 계획 가이드 _"^] 및  https://library.netapp.com/ecm/ecm_download_file/ECMLP2856176["_NetApp HCI 설치 및 설정 지침 _"^]을 참조하십시오.
...
* 각 스토리지 노드에 사용되는 스위치 포트에 대한 LACP(Link Aggregation Control Protocol)를 구성하는 방법에 대한 자세한 내용은 을 참조하십시오.link:hci_prereqs_LACP_configuration.html["최적의 스토리지 성능을 위해 LCAP를 구성합니다"]
...
필수 구성 요소에 대한 자세한 내용은 을 참조하십시오link:hci_prereqs_overview.html["NetApp HCI 구축 개요에 대한 요구 사항"].
...
NetApp HCI에 대한 네트워크 준비를 위해 NetApp Active IQ Config Advisor 5.8.1 이상을 설치합니다. 이 네트워크 유효성 검사 도구는 기타 와 함께 link:https://mysupport.netapp.com/site/tools/tool-eula/5ddb829ebd393e00015179b2["NetApp 지원 툴"^]있습니다. 이 툴을 사용하여 연결, VLAN ID, IP 주소 요구 사항, 스위치 연결 등을 확인할 수 있습니다.
...
CAUTION: 예방 조치 및 자세한 내용은 을 link:task_hci_installhw.html["H 시리즈 하드웨어를 설치합니다"]참조하십시오.
...
을 link:hci_prereqs_final_prep.html["IPMI를 구성합니다"]참조하십시오.
...
.단계
. 초기 스토리지 노드 관리 네트워크 IP를 모르는 경우 스토리지 노드 또는 의 키보드와 모니터를 통해 액세스하는 TUI(터미널 사용자 인터페이스)를 link:task_nde_access_dhcp.html["USB 스틱을 사용하십시오"]사용합니다.
+
...
. 새 vCenter 구축을 구성합니다. Configure using a fully Qualified Domain Name * 을 선택하고 vCenter Server 도메인 이름과 DNS Server IP 주소를 모두 입력합니다.
+
...
. 모든 노드를 선택하고 * Continue * 를 선택합니다.
. 네트워크 설정을 구성합니다. 사용할 값은 _NetApp HCI 설치 검색 워크북_을 참조하십시오.
. 파란색 상자를 선택하여 Easy 양식을 시작합니다.
+
...
.. VLAN ID를 할당하시겠습니까? 에 대해 * 아니요 * 를 선택합니다. (나중에 기본 네트워크 설정 페이지에서 할당합니다.)
...
+
사용자 이름에 을 추가해야 `@vsphere.local` 합니다.
...
* https://www.netapp.com/pdf.html?item=/media/9413-tr4820.pdf["TR-4820: NetApp HCI 네트워킹 빠른 계획 가이드"^]
* https://docs.netapp.com/us-en/vcp/index.html["vCenter Server용 NetApp Element 플러그인 가이드 를 참조하십시오"^]
* https://mysupport.netapp.com/site/tools/tool-eula/5ddb829ebd393e00015179b2["NetApp 구성 어드바이저"^] 5.8.1 이상 네트워크 검증 도구
