...
| 더 적합하였습니다 | ISO 
|===
* (프록시 서버가 있는 관리 노드 12.0 및 12.2) 프록시 서버를 구성하기 전에 NetApp 하이브리드 클라우드 제어를 관리 서비스 버전 2.16으로 업데이트했습니다.


.이 작업에 대해
Element 12.2 관리 노드는 선택적 업그레이드입니다. 기존 배포에는 필요하지 않습니다.

이 절차를 따르기 전에 해당 기능을 사용할 것인지 및 사용할 것인지 여부를 이해해야 link:concept_hci_volumes.html#persistent-volumes["영구 볼륨"]합니다. 영구 볼륨은 선택 사항이지만 가상 머신(VM)이 손실된 경우 관리 노드 구성 데이터 복구에 권장됩니다.
...
. NetApp 지원 사이트의 페이지에서 설치에 필요한 OVA 또는 ISO를 https://mysupport.netapp.com/site/products/all/details/netapp-hci/downloads-tab["NetApp HCI"^] 다운로드합니다.
+
...
.. 기본 시간 서버를 추가하려면 각 서버마다 새 줄을 추가하십시오. 기본 시간 서버는 link:task_mnode_install.html#step-4-set-up-the-management-node["나중에"]에서 사용할 스토리지 클러스터에서 사용하는 NTP 서버와 동일해야 합니다.
+
[listing]
----
vi /etc/ntp.conf
...
NOTE: 시간 동기화 구성을 완료한 후에는 NTP를 편집하지 마십시오. 관리 노드에서 link:task_mnode_install.html#step-4-set-up-the-management-node["설정 명령"]을(를) 실행할 때 NTP에 영향을 미칩니다.
...
*** *--storage_mvip (-sm) [MVIP 주소]*: Element 소프트웨어가 실행되는 스토리지 클러스터의 관리 가상 IP 주소(MVIP)입니다. link:task_mnode_install.html#step-3-configure-time-sync["NTP 서버 구성"] 중에 사용한 것과 동일한 스토리지 클러스터로 관리 노드를 구성하십시오.
...
NOTE: 컨트롤러 하위 자산을 추가하려면 vCenter에서 새로운 NetApp HCC 역할을 생성하는 것이 좋습니다. 이러한 새로운 NetApp HCC 역할은 관리 노드 서비스 뷰를 NetApp 전용 자산으로 제한합니다. 을 link:task_mnode_create_netapp_hcc_role_vcenter.html["vCenter에서 NetApp HCC 역할을 생성합니다"]참조하십시오.
...
.. 체험하기 * 를 선택합니다.
.. 클립보드에 복사한 상위 기본 자산 ID를 * asset_id * 필드에 입력합니다.
.. 페이로드에서 Model(모델) 탭에 정의된 대로 필요한 페이로드 값을 입력합니다. 으로 `type` 를 `ESXi Host` 입력하고 이전 단계에서 저장한 하드웨어 태그를 `hardware_tag` 입력합니다.
.. Execute * 를 선택합니다.
