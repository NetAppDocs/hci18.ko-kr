...
다음 단계에 따라 컨트롤러 자산을 업데이트합니다. 을 link:task_mnode_edit_vcenter_assets.html["기존 컨트롤러 자산을 보거나 편집합니다"]참조하십시오.
...
.단계
. 관리 노드 REST API 사용자 인터페이스에 대한 개요를 보려면 를 참조하십시오link:task_mnode_work_overview_API.html["관리 노드 REST API 사용자 인터페이스 개요"].
...
. REST API UI에서 * Get ​/assets/compute_nodes * 를 선택합니다.
+
...
. 특정 컴퓨팅 노드 자산을 구성합니다.
+
.. Put/assets/{asset_id}/compute-nodes/{compute_id} * 를 선택합니다.
+
