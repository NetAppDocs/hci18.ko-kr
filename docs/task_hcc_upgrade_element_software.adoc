...
IMPORTANT: Element 소프트웨어를 버전 12로 업그레이드하기 전에 최신 관리 서비스 번들로 업그레이드해야 합니다. Element 소프트웨어를 버전 12.2로 업데이트하는 경우 계속하려면 관리 서비스 2.14.60 이상이 필요합니다.
...
* * H610S 노드의 업데이트된 BMC *: H610S 노드의 BMC 버전을 업그레이드했습니다. 를 link:rn_H610S_BMC_3.84.07.html["릴리스 정보 및 업그레이드 지침"]참조하십시오.
...
IMPORTANT: H610S 시리즈 노드를 Element 12.2로 업그레이드하는 중이며 노드에서 Element 11.8 이전 버전을 실행 중인 경우 각 스토리지 노드에 대해 추가 업그레이드 단계()를 수행해야<<H610S 스토리지 노드를 Element 12.2로 업그레이드(2단계),2단계>> 합니다. Element 11.8 이상을 실행 중인 경우 추가 업그레이드 단계(2단계)가 필요하지 않습니다.
...
.. 에서 필요한 추가 단계(2단계)를 https://kb.netapp.com/Advice_and_Troubleshooting/Hybrid_Cloud_Infrastructure/H_Series/NetApp_H610S_storage_node_power_off_and_on_procedure["KB 문서를 참조하십시오"^] 완료하고 UI에서 2단계를 완료했음을 확인합니다.
...
| 후속 조치를 완료합니다 | 11.8 이전의 Element 버전에서 H610S 노드를 업그레이드하는 경우에만 해당됩니다. 업그레이드 프로세스의 1단계가 완료되면 이 상태는 업그레이드의 2단계를 수행하라는 메시지를 표시합니다(참조 https://kb.netapp.com/Advice_and_Troubleshooting/Hybrid_Cloud_Infrastructure/H_Series/NetApp_H610S_storage_node_power_off_and_on_procedure["KB 문서를 참조하십시오"^]). 2단계를 완료하고 완료했음을 확인한 후 상태가 * 최신 * 로 변경됩니다. 
|===
...
.단계
. 관리 노드에서 액세스할 수 있는 디바이스에 스토리지 업그레이드 패키지를 다운로드합니다. NetApp HCI 소프트웨어로 이동하여 최신 스토리지 노드 이미지를 다운로드합니다. https://mysupport.netapp.com/site/products/all/details/netapp-hci/downloads-tab["다운로드 페이지"^]
...
| Element 11.8 이전 버전을 실행 중인 H610S 클러스터를 업그레이드하는 경우 응답 본문에 상태가 표시됩니다 `finishedNeedsAck`. 각 H610S 스토리지 노드에 대해 추가 업그레이드 단계(2단계)를 수행해야 합니다.  a| 
... 를 참조하여 <<Upgrading H610S storage nodes to Element 12.2 or later (phase 2)>>각 노드에 대한 프로세스를 완료합니다.
... 필요한 경우 재인증을 수행하고 * Put cer/upgrades/{upgrade eId} * 를 선택합니다.
... 체험하기 * 를 선택합니다.
... 이전 단계의 업그레이드 ID를 * upgrade eId * 에 입력합니다.
...
.단계
. 스토리지 업그레이드 패키지를 다운로드합니다. NetApp HCI 소프트웨어로 https://mysupport.netapp.com/site/products/all/details/netapp-hci/downloads-tab["다운로드 페이지"^] 이동하여 관리 노드가 아닌 디바이스에 최신 스토리지 노드 이미지를 다운로드합니다.
+
...
NOTE: 정보를 묻는 메시지가 표시되도록 명령줄에서 암호를 생략합니다. `sfinstall` 특수 문자가 포함된 암호의 경우 (`\`각 특수 문자 앞에 백슬래시)를 추가합니다. 예를 들어, 는 `mypass!@1` 로 `mypass\!\@`입력해야 합니다.
...
----
+
샘플의 출력은 의 최신 버전이 사용 가능한지 여부를 확인하려는 시도를 `sfinstall` 보여줍니다 `sfinstall`.
...
NOTE: 스테이징이 완료되면 업그레이드 이벤트 후에 메시지가 `Storage Node Upgrade Staging Successful` 표시됩니다.
...
. ISO 파일의 경로와 명령을 사용하여 업그레이드를 시작합니다 `sfinstall`.
+
...
HealthTools 도구 제품군을 사용하여 다크 사이트에서 NetApp Element 소프트웨어를 업데이트할 수 있습니다.

.필요한 것
. NetApp HCI 소프트웨어로 https://mysupport.netapp.com/site/products/all/details/netapp-hci/downloads-tab["다운로드 페이지"^]이동합니다.
. 올바른 소프트웨어 릴리즈를 선택하고 최신 스토리지 노드 이미지를 관리 노드가 아닌 컴퓨터에 다운로드합니다.
+

NOTE: Element 스토리지 소프트웨어를 업그레이드하려면 최신 버전의 HealthTools가 필요합니다.

. 관리 노드가 아닌 컴퓨터의 NetApp 지원 사이트에서 이 파일을 https://library.netapp.com/ecm/ecm_get_file/ECMLP2840740["JSON 파일"^] 다운로드하고 이름을 로 변경합니다 `metadata.json`.
. 와 같은 액세스 가능한 위치에 있는 관리 노드에 ISO `/tmp` 파일을 복사합니다.
+
...
.단계
.  `sfupdate-healthtools`다음 명령을 실행합니다.
+
[listing]
----
...
----
. ISO 파일 및 메타데이터 JSON 파일에 대한 경로로 명령을 실행합니다 `sfinstall`.
+
[listing]
----
...
----
+
* 선택 사항 * 명령에 플래그를 `sfinstall` 추가하여 업그레이드를 미리 준비할 수 `--stage` 있습니다.
...
TIP: Ctrl-C만 사용하여 업그레이드를 일시 중지해야 합니다. 이렇게 하면 시스템이 자동으로 정리됩니다.
 ...
 `sfinstall`클러스터 장애가 삭제될 때까지 대기하고 장애가 남아 있는 경우 는 `sfinstall` 다음 노드로 진행하지 않습니다.

.단계
. Ctrl+C로 중단해야 합니다 `sfinstall`
. 장애 조사에 대한 도움을 받으려면 NetApp 지원에 문의하십시오.
. 동일한 명령으로 업그레이드를 재개합니다. `sfinstall`
...
== H610S 스토리지 노드를 Element 12.2로 업그레이드(2단계)

H610S 시리즈 노드를 Element 12.2로 업그레이드하는 중이며 노드에서 Element 11.8 이전 버전을 실행 중인 경우 업그레이드 프로세스에는 2단계가 포함됩니다.

먼저 수행되는 1단계는 Element 12.2 프로세스의 표준 업그레이드와 동일한 단계를 따릅니다. Element 소프트웨어와 5개의 펌웨어 업데이트를 한 번에 한 노드씩 클러스터에 모두 설치합니다. 펌웨어 페이로드로 인해 H610S 노드당 프로세스는 각 노드의 업그레이드 종료 시 단일 콜드 부팅 주기를 포함하여 약 1.5~2시간이 소요될 것으로 예상됩니다.

2단계에서는 필수 에 설명된 각 H610S 노드에 대해 전체 노드 종료 및 전원 연결을 수행하는 단계를 https://kb.netapp.com/Advice_and_Troubleshooting/Hybrid_Cloud_Infrastructure/H_Series/NetApp_H610S_storage_node_power_off_and_on_procedure["KB를 클릭합니다"^] 완료합니다. 이 단계는 H610S 노드당 약 1시간이 소요될 것으로 예상됩니다.
...
. 각 H610S 노드에 대해 업그레이드를 완료하는 데 필요한 2단계 업그레이드 절차를 https://kb.netapp.com/Advice_and_Troubleshooting/Hybrid_Cloud_Infrastructure/H_Series/NetApp_H610S_storage_node_power_off_and_on_procedure["KB를 클릭합니다"^] 따릅니다.
